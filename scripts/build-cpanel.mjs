...
//   CPANEL_DB_USER=<cpanelprefix>_arc CPANEL_DB_NAME=<cpanelprefix>_arc \
//   CPANEL_OWNER_EMAIL=admin@abdulrazakcatering.com CPANEL_OWNER_PASSWORD=<a real one> \
//   node scripts/build-cpanel.mjs
...
// Deploy-time knobs. The database names are placeholders that the deploy notes
// tell you to replace — cPanel prefixes every database with the account name,
// so the real one is only known once the hosting account exists.
const DB_USER = process.env.CPANEL_DB_USER || 'CPANELPREFIX_arc';
const DB_NAME = process.env.CPANEL_DB_NAME || 'CPANELPREFIX_arc';
const DB_HOST = process.env.CPANEL_DB_HOST || 'localhost';
const DB_PORT = process.env.CPANEL_DB_PORT || '3306';
const OWNER_EMAIL = process.env.CPANEL_OWNER_EMAIL || 'admin@abdulrazakcatering.com';
/**
 * The first sign-in password.
 *
 * Generated fresh on every build rather than defaulted to a literal, so a
 * build run without the override cannot ship a password that is written down
 * in this repository. It is printed by the build and written into the deploy
 * notes; the notes tell you to change it on first sign-in either way.
 */
const OWNER_PASSWORD = process.env.CPANEL_OWNER_PASSWORD || `arc-${randomBytes(6).toString('hex')}`;
...
`,
  ...
  // 001 IS the current schema — it was consolidated to the final shape, so a
  // fresh install is 001 alone. The loop above still runs because any later
  // migration added from here on has to reach a new install too, and finding
  // that out after the site is up is expensive.
  ...
  const ownerSql =
    ...
    `--\n` +
    `-- Seeded as SUPER_ADMIN, not OWNER: managing user accounts is carved out\n` +
    `-- of the Owner's access, so a fresh install with only an Owner would have\n` +
    `-- nobody able to create the second login. Create the Owner from Users.\n` +
  ...
  await writeFile(path.join(out, 'database', 'abdulrazakcatering-schema.sql'), schemaSql);
  await writeFile(path.join(out, 'database', 'abdulrazakcatering-with-admin.sql'), ownerSql);
...
function deployMd() {
  return `# Deploying Abdul Razak Catering Service to cPanel
...
- \`database/abdulrazakcatering-schema.sql\` — the complete current schema, empty (FIRST install only)
- \`database/abdulrazakcatering-with-admin.sql\` — same tables **+ one SUPER ADMIN login** (FIRST install only)
...
final name/user/password (cPanel prefixes them, e.g. \`usaaswtj_abdulrazakcatering\`).

## 2. Import the tables (cPanel → phpMyAdmin)
Select the database → **Import** → choose \`database/abdulrazakcatering-with-admin.sql\` → Go.
(That file also creates the sign-in account below. Use \`abdulrazakcatering-schema.sql\` instead
only if you'll create that account yourself.)

## 3. Upload the app (cPanel → File Manager)
Create a folder **outside** \`public_html\` (e.g. \`abdulrazakcatering\`), upload
