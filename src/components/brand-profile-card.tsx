 ...
 *
 * The public identity: what the website's contact section shows, what the
 * call and WhatsApp buttons dial, and what Google puts in the tab. A typo here
 * reaches customers, so the form says which field goes where.
 */
    ...
    <FadeUp>
      <Card className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h3 className="flex items-center gap-2 font-display text-lg text-gold">
              <Building2 className="h-5 w-5" /> Website &amp; Contact
            </h3>
            <p className="mt-0.5 max-w-xl text-sm text-[rgb(var(--text-muted))]">
              The public identity. These fill the contact section on the website, dial
              the call and WhatsApp buttons, and appear in the browser tab.
            </p>
          </div>
        </div>
        ...
        <p className="mt-3 text-xs text-[rgb(var(--text-dim))]">
          A field left empty is simply not shown. The website hides a button it cannot
          dial, and the printed slip drops the line rather than printing a label with
          nothing after it.
        </p>
      </Card>
    </FadeUp>
