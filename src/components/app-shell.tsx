...
import { CATERING_NAV, type NavItem } from '@/lib/nav';
...
export function AppShell({
  user, children,
  // Defaulted rather than required: there is one portal, and every caller
  // wants the same sidebar. They stay parameters because the shell is also
  // what a second portal would be built on.
  nav = CATERING_NAV,
  homeHref = '/catering',
  brandName,
  brandSub,
...
function UserMenu({ user, theme, onToggleTheme }: { user: { name?: string | null; role: Role }; theme: string; onToggleTheme: () => void }) {
  return (
    <div className="space-y-2">
      <Link href="/catering/account" className="flex items-center gap-3 rounded-xl px-2 py-1.5 transition-colors hover:bg-[rgb(var(--surface-2))]">
