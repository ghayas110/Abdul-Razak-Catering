...
/**
 * Super Admins implicitly hold every permission, which also makes this robust
 * against a stale JWT issued before a permission was added. Everyone else is
 * judged on their resolved list — see `effectiveCan` in permissions.ts.
 */
...
export async function requirePermission(perm: Permission): Promise<SessionUser> {
  ...
  if (!hasPermission(user, perm)) redirect('/catering?denied=' + perm);
  return user;
}
