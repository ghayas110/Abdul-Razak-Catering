 ...
 * Catering — read layer.
 *
 * Everything the portal reads: customers, the priced menu, quotations and the
 * invoices raised from them, the vendors those events are bought from, and the
 * ledger and reports that fall out of the two.
 */
 ...
 * prefix so the client can edit it from Settings without a deploy. Falls back
 * to the defaults in types.ts when a key has never been saved.
 */
...
export async function getCateringMenu(activeOnly = false): Promise<CateringMenuItemRow[]> {
  ...
  const vRows = await query<any>(
    `SELECT v.menu_item_id, v.category_id, v.rate, c.name AS category_name
...
export async function getCateringLines(quotationId: number): Promise<CateringLineRow[]> {
  const rows = await query<any>(
    `SELECT * FROM catering_quotation_items
      WHERE quotation_id = ?
      ORDER BY FIELD(section,'DISH','CHARGE'), sort_order, id`,
    [quotationId],
  );
  return rows.map((r) => ({
...
export interface CateringReportRow {
  ...
  docNo: string;
  invoiced: boolean;
  /** Dishes and charges — every billed line on the document. */
  items: number;
  total: number;
  vendor: number;
  received: number;
...
/**
 * One row per EVENT for a period: what was billed, then what it cost and what
 * it made.
 *
...
export async function getCateringReport(from: string, to: string, label: string): Promise<CateringReport> {
  const rows = await query<any>(
    `SELECT q.id, q.quota_no, q.customer_name, q.delivery_date, q.quotation_date,
            q.items_total AS q_items, q.grand_total AS q_total,
            inv.id AS invoice_id, inv.quota_no AS invoice_no,
            inv.items_total AS i_items, inv.grand_total AS i_total,
  ...
  const totals = out.reduce(
    (a, r) => ({
      items: a.items + r.items, total: a.total + r.total,
      vendor: a.vendor + r.vendor, received: a.received + r.received, profit: a.profit + r.profit,
    }),
    { items: 0, total: 0, vendor: 0, received: 0, profit: 0 },
  );
