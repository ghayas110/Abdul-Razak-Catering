...
const menuSchema = z.object({
  ...
  // (GRAM, ML) and is converted before the amount is worked out.
...
export async function saveCateringMenuItem(id: number | null, input: unknown): Promise<CateringResult> {
  ...
  try {
    const itemId = await withTransaction(async (tx) => {
      ...
      for (const v of d.variants) {
        await tx.execute(
          `INSERT INTO catering_menu_item_categories (menu_item_id, category_id, rate, is_active) VALUES (?,?,?,1)`,
          [out, v.categoryId, v.rate],
        );
      }
      return out!;
    });
        ...
        await tx.execute(
          `UPDATE catering_quotations
              SET customer_id=?, customer_name=?, contact_no=?, place_of_function=?,
                  ...
                  items_total=?, grand_total=?, advance_amount=?, status=?, note=?
            WHERE id=?`,
          [d.customerId ?? null, d.customerName, d.contactNo, d.placeOfFunction,
           d.quotationDate, d.deliveryDate || null, d.persons,
           itemsTotal, grandTotal, d.advanceAmount, d.status, d.note || null, out],
        );
        ...
        const res = await tx.execute(
          `INSERT INTO catering_quotations
             (quota_no, customer_id, customer_name, contact_no, place_of_function,
              quotation_date, delivery_date, persons, items_total, grand_total,
              advance_amount, paid_amount, status, note, created_by)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,0,?,?,?)`,
          [quotaNo, d.customerId ?? null, d.customerName, d.contactNo, d.placeOfFunction,
           d.quotationDate, d.deliveryDate || null, d.persons,
           itemsTotal, grandTotal, d.advanceAmount, d.status, d.note || null, actor.id],
        );
        out = res.insertId;
      }
      ...
      for (const l of lines) {
        await tx.execute(
          `INSERT INTO catering_quotation_items
             (quotation_id, section, menu_item_id, description, category, category_id, qty, unit, rate, amount, sort_order)
           VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
          [out, l.section, l.menuItemId ?? null, l.description, l.category, l.categoryId ?? null,
           l.qty, l.unit, l.rate, l.amount, n++],
        );
      }
      return out!;
    });
      ...
      const res = await tx.execute(
        `INSERT INTO catering_quotations
           (quota_no, doc_type, source_quotation_id, customer_id, customer_name, contact_no,
            ...
            items_total, grand_total, advance_amount, paid_amount, status, note, created_by)
         SELECT ?, 'INVOICE', ?, customer_id, customer_name, contact_no,
                place_of_function, CURDATE(), delivery_date, persons,
                items_total, grand_total, advance_amount, 0, 'CONFIRMED', note, ?
      ...
      await tx.execute(
        `INSERT INTO catering_quotation_items
           (quotation_id, section, menu_item_id, description, category, category_id,
            qty, unit, rate, amount, sort_order)
      ...
      for (const l of d.lines) {
        ...
        await tx.execute(
          `INSERT INTO catering_template_items
             (template_id, section, menu_item_id, description, category, category_id, qty, unit, rate, amount, sort_order)
           VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
          [out, l.section, l.menuItemId ?? null, l.description, l.category ?? '', l.categoryId ?? null,
           l.qty, l.unit, l.rate, amount, n++],
        );
      }
      return out;
    });
  ...
  try {
    const templateId = await withTransaction(async (tx) => {
      ...
      await tx.execute(
        `INSERT INTO catering_template_items
           (template_id, section, menu_item_id, description, category, category_id, qty, unit, rate, amount, sort_order)
         SELECT ?, section, menu_item_id, description, category, category_id, qty, unit, rate, amount, sort_order
           FROM catering_quotation_items WHERE quotation_id = ?`,
        [res.insertId, quotationId],
      );
      return res.insertId;
