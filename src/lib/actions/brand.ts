 ...
 *
 * Each field is one row in `settings`. Clearing one writes an empty string
 * rather than deleting the row, and every surface that shows these values
 * hides the line when it is empty — see the note on BRAND_DEFAULTS. Use
 * `resetBrand` to actually go back to the shipped defaults.
 */
