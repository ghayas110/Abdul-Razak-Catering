...
import { UNIT_META, type CateringCategoryRow, type CateringMenuItemRow, type CateringUnit } from '@/lib/types';
import { Plus, Pencil, Trash2, Archive, ArchiveRestore, UtensilsCrossed, X, Tags } from 'lucide-react';

/** Only the base units a rate can be quoted in — GRAM and ML are order units. */
const BASE_UNITS: CateringUnit[] = ['KG', 'LITRE', 'PCS', 'PLATE'];

type Variant = { key: string; categoryId: number | null; rate: string };
const uid = () => Math.random().toString(36).slice(2);

export function MenuClient({
  items, categories, canManage,
}: { items: CateringMenuItemRow[]; categories: CateringCategoryRow[]; canManage: boolean }) {
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              ...
              <Button variant="ghost" onClick={() => setVariants((v) => [...v, { key: uid(), categoryId: null, rate: '' }])}>
