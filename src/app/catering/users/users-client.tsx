...
const ROLE_TONE: Record<Role, 'gold' | 'green' | 'amber' | 'muted'> = {
  SUPER_ADMIN: 'gold', OWNER: 'gold', MANAGER: 'green',
  ACCOUNTANT: 'amber', VIEWER: 'muted',
};
...
function StaffTag({ u }: { u: UserVM }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs text-positive" title={`On the payroll as ${u.designation || 'Staff'}`}>
    ...
    <Modal open onClose={onClose} title={isEdit ? `Edit ${initial!.name}` : 'Create user'} wide>
      <div className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Email"><Input type="email" value={email} disabled={isEdit} onChange={(e) => setEmail(e.target.value)} placeholder="name@abdulrazakcatering.pk" /></Field>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
